...
$rows=array();
$sql="SELECT c.id,c.title,c.price,c.status,c.car_year,c.location,cat.name category
      FROM cars c
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table align-middle">
          <thead>
            <tr>
              <th>ID</th><th>Title</th><th>Category</th><th>Year</th><th>Location</th><th>Price</th><th>Status</th><th class="text-end">Actions</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach($rows as $x): ?>
              <tr>
                ...
                <td><span class="badge text-bg-<?php echo badge_class($x['status']); ?>"><?php echo esc($x['status']); ?></span></td>
                <td class="text-end">
                  ...
                  <a class="btn btn-sm btn-outline-danger ms-1" onclick="return confirm('Soft delete this car?');" href="<?php echo BASE_URL; ?>admin/cars/delete.php?id=<?php echo (int)$x['id']; ?>">Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
