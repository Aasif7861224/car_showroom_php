...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $stmt=db()->prepare("INSERT INTO cars (category_id,title,brand,model,car_year,fuel,transmission,price,mileage_km,location,description,status,is_deleted,is_featured,discount_percent,view_count)
                       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,0,0,0)");
  $stmt->bind_param("isssissdisss",$category_id,$title,$brand,$model,$car_year,$fuel,$transmission,$price,$mileage_km,$location,$description,$status);
  $stmt->execute();

  $car_id=db()->insert_id;
  $uploaded=upload_images_for_car($car_id);

  flash_set('ok','Car added. Images uploaded: '.$uploaded);
...
<div class="container my-4">
  <div class="d-flex justify-content-between align-items-end flex-wrap gap-2">
    <div><h3 class="fw-bold mb-1">Add Car</h3><p class="text-muted mb-0">Create car + upload images.</p></div>
  ...
  <div class="card card-soft mt-3">
    <div class="card-body">
      <form method="post" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">Brand *</label><input class="form-control" name="brand" required></div>
        <div class="col-md-4"><label class="form-label">Model *</label><input class="form-control" name="model" required></div>
        <div class="col-md-4"><label class="form-label">Year *</label><input class="form-control" type="number" name="car_year" required></div>

        <div class="col-md-4">
          <label class="form-label">Fuel *</label>
          <select class="form-select" name="fuel" required>
            <option value="">Select</option><option>Petrol</option><option>Diesel</option><option>CNG</option><option>Electric</option><option>Hybrid</option>
          </select>
        </div>
        <div class="col-md-4">
          <label class="form-label">Transmission *</label>
          <select class="form-select" name="transmission" required>
            <option value="">Select</option><option>Manual</option><option>Automatic</option>
          </select>
        </div>
        <div class="col-md-4"><label class="form-label">Price (₹) *</label><input class="form-control" type="number" name="price" required></div>

        <div class="col-md-4"><label class="form-label">Mileage (km)</label><input class="form-control" type="number" name="mileage_km"></div>
        <div class="col-md-4"><label class="form-label">Location *</label><input class="form-control" name="location" required></div>
        <div class="col-md-4">
          <label class="form-label">Status</label>
          <select class="form-select" name="status"><option>Draft</option><option>Published</option><option>Sold</option></select>
        </div>
        ...
        <div class="col-12">
          ...
          <div class="text-muted small mt-1">First image becomes Primary (if none exists).</div>
        </div>
