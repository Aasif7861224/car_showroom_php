...
$rows=array();
$sql="SELECT o.id,u.full_name,u.email,c.title car_title,o.amount,o.status,o.created_at
      FROM orders o
...
<div class="container-fluid my-4 px-3 px-md-4">
  <div class="d-flex justify-content-between align-items-end flex-wrap gap-2">
    <div><h3 class="fw-bold mb-1">Orders</h3><p class="text-muted mb-0">Manage order lifecycle & status updates.</p></div>
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table align-middle">
          <thead>
            <tr><th>ID</th><th>Customer</th><th>Car</th><th>Amount</th><th>Status</th><th>Date</th><th class="text-end">Actions</th></tr>
          </thead>
          <tbody>
            <?php foreach($rows as $x): ?>
              <tr>
                ...
                <td><span class="badge text-bg-<?php echo badgeOrder($x['status']); ?>"><?php echo esc($x['status']); ?></span></td>
                <td class="text-muted small"><?php echo esc($x['created_at']); ?></td>
                <td class="text-end">
                  <a class="btn btn-sm btn-outline-primary me-2" href="<?php echo BASE_URL; ?>admin/orders/view.php?id=<?php echo (int)$x['id']; ?>">View</a>
                  <form class="d-inline-flex gap-2 align-items-center" method="post" action="<?php echo BASE_URL; ?>admin/orders/update_status.php">
            ...
            <?php endforeach; ?>
            <?php if(count($rows)===0): ?><tr><td colspan="7" class="text-muted">No orders.</td></tr><?php endif; ?>
          </tbody>
        </table>
      </div>
    </div>
