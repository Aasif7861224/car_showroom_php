...
if($req['status']!=='Pending'){ flash_set('err','Only Pending can be approved.'); redirect_to('admin/sell_requests/index.php'); }
...
$mileage_km=(int)($req['mileage_km'] ?? 0);
$location=$req['location'];
$description='Added from Sell Request (#'.$req['id'].'). Update details + add images, then publish.';
$status='Draft';
...
$stmt2=db()->prepare("INSERT INTO cars (category_id,title,brand,model,car_year,fuel,transmission,price,mileage_km,location,description,status,is_deleted,is_featured,discount_percent,view_count)
                      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,0,0,0)");
$stmt2->bind_param("isssissdisss",$category_id,$title,$brand,$model,$car_year,$fuel,$transmission,$price,$mileage_km,$location,$description,$status);
$stmt2->execute();
...
flash_set('ok','Approved! Car added as Draft. Now edit, upload images, and publish.');
