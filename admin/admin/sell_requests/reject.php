...
if ($req['status'] !== 'Pending') { flash_set('err','Only Pending requests can be rejected.'); redirect_to('admin/sell_requests/index.php'); }

if ($_SERVER['REQUEST_METHOD']==='POST') {
  $reason = trim(isset($_POST['reason']) ? $_POST['reason'] : '');
  if ($reason==='') {
    flash_set('err','Reject reason is required.');
    redirect_to('admin/sell_requests/reject.php?id='.$id);
  }
